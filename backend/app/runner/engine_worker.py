...
def main() -> int:
    ...
    if cfg_in.get("fno_mode") and cfg_in.get("_fno_context"):
        ...
        plan_prompt = (
            ...
            "in markdown.\n\n"
            "The chain table includes computed Greeks per strike: (Δ delta / θ theta-per-day / IV).\n\n"
            "Requirements:\n"
            "- Start with '## Option Trade Plan'.\n"
            "- Choose the specific strike and side (CE/PE) with the best risk/reward for the "
            "rating — justify the choice explicitly via DELTA (0.35–0.55 |Δ| preferred for "
            "directional longs), IV richness vs India VIX (rich IV ⇒ prefer debit spreads), "
            "theta cost per day, OI walls, max pain, and the technical picture.\n"
            "- State the position's delta exposure and daily theta bleed, and the breakeven.\n"
